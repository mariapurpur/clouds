...
## Плохие practicies по работе с контейнерами
  
  Немного разобравшись, мы выделили следующие practicies:  
  
**Запуск контейнеров с правами суперпользователя (root).**  
  Если так делать, то это увеличивает риск эксплуатации уязвимостей, так как злоумышленник может получить доступ ко всем root-привилегиям. Лучше ограничить права доступа, изолировать контейнеры и регулярно сканировать безопасность (по возможности).  

**Запуск нескольких процессов в одном контейнере.**    
  Это особенно плохая practice, потому что она может привести к тому, что ошибка, которая возникла в одном процессе, может легко перейти на другой и управление ресурсами станет сложнее.  
